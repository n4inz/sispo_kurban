...
class KeuanganService{
    ...
    public function index($filter = [])
    {
        ...
        $query = UserPaketKurban::whereHas('user')->with('user')->orderBy('created_at', $orderBy);

        if ($status !== null) {
            $query->whereIn('status_paid', $status)->whereHas('user');
        }

        if ($name !== null) {
            $query->where('status_paid', 'LIKE','%'.$name.'%');
        }
